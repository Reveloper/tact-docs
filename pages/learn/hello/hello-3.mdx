...
Come back to `tact-guide-template` project via IDE(VScode) and begin to write the contract file `contract.tact`. That, our first Tact string is declaring an additional library, where some useful Tact functions are defined.

```tact filename="sources/contract.tact" copy
...
As said before `Address` defined in the [Tact Type system](/guides/types) that describes smart-contract's language. General information about Address in TON placed [here](https://ton.org/docs/learn/overviews/addresses).
