...
* **Wallet app**. You need one of TON noncustodial testnet [wallet app](https://ton.org/docs/participate/wallets/apps) (better with support Walletv4), for example [Sandbox](https://test.tonhub.com/)/[Tonkeeper](https://tonkeeper.com/). This is as part of simplify demonstration, you also can get access to your wallet from code, example added in demo. 
...
This project has ready to use TACT compiler, typescript + jest with [ton-emulator](https://github.com/ton-core/ton-emulator), example how to test and deploy.
...
3. File `wallet.deploy.ts` is a helper, that allow to deploy your `wallet.tact` compiled file(src/output) with deployment link. From the beginning you can deploy your smart contract via [Sandbox](https://test.tonhub.com/)/[Tonkeeper](https://tonkeeper.com/) application.
4. Describes alternative deployment script `wallet.deploy-api.ts` for `yarn deploy-api` according to your `contract.tact` to send deployment message from deployment wallet. You need to input your deployment wallet 24 words [here](https://github.com/ton-community/tact-wallet/blob/main/sources/wallet.deploy-api.ts#L19).
